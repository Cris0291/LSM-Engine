...
private:
  ...
  std::uint32_t from_4_bytes_little_endian(std::span<std::byte> bytes_to_order);

public:
  std::vector<std::byte> encode(OperationRecord ops, std::span<std::byte> keys,
                                std::span<std::byte>);
  DecodeResult decode(std::span<std::byte> record);
};
